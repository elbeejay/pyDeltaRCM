...
class TestModelDomainSetup:
    def test_inlet_size_specified(self, tmp_path: Path) -> None:
        file_name = "user_parameters.yaml"
        p, f = utilities.create_temporary_file(tmp_path, file_name)
        utilities.write_parameter_to_file(f, "out_dir", tmp_path / "out_dir")
        utilities.write_parameter_to_file(f, "Length", 4000.0)
        utilities.write_parameter_to_file(f, "Width", 8000.0)
    ...
    def test_inlet_size_set_to_one_fourth_domain(self, tmp_path: Path) -> None:
        file_name = "user_parameters.yaml"
        p, f = utilities.create_temporary_file(tmp_path, file_name)
        utilities.write_parameter_to_file(f, "out_dir", tmp_path / "out_dir")
        utilities.write_parameter_to_file(f, "Length", 4000.0)
    ...
    def test_x(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.x[0][-1] == 199

    def test_y(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.y[0][-1] == 0

    def test_cell_type(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        # wall type in corner
        assert delta.cell_type[0, 0] == -2

    def test_eta(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.eta[10, 10] == -5

    def test_stage(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
    ...
    def test_qx(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        # prescribe the qx at the inlet
        assert delta.qx[0, delta.CTR] == 5

    def test_qy(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.qy[0, delta.CTR] == 0

    def test_qxn(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.qxn[0, 0] == 0

    def test_qyn(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.qyn[0, 0] == 0

    def test_qwn(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.qwn[0, 0] == 0

    def test_ux(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.ux[0, delta.CTR] == 1

    def test_uy(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.uy[0, delta.CTR] == 0

    def test_uw(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.uw[0, delta.CTR] == 1

    def test_qs(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.qs[5, 5] == 0

    def test_Vp_dep_sand(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert np.any(delta.Vp_dep_sand) == 0

    def test_Vp_dep_mud(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert np.any(delta.Vp_dep_mud) == 0

    def test_free_surf_flag(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert np.any(delta.free_surf_flag) == 0

    def test_indices(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert np.any(delta.free_surf_walk_inds) == 0

    def test_sfc_visit(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert np.any(delta.sfc_visit) == 0

    def test_sfc_sum(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
...
class TestCreateBoundaryConditions:
    ...
    def test_change_variable_updated_bcs(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
...
class TestInitSubsidence:
    def test_subsidence_bounds(self, tmp_path: Path) -> None:
...
class TestLoadCheckpoint:
    ...
    def test_load_standard_grid(self, patched, tmp_path: Path) -> None:
        ...
        assert os.path.isfile(os.path.join(_delta.prefix, "pyDeltaRCM_output.nc"))
    ...
    def test_load_wo_netcdf_not_expected(self, patched, tmp_path: Path) -> None:
        ...
        assert os.path.isfile(os.path.join(_delta.prefix, "checkpoint.npz"))
        assert not os.path.isfile(os.path.join(_delta.prefix, "pyDeltaRCM_output.nc"))
    ...
    def test_check_all_fields_present(self, patched, tmp_path: Path) -> None:
        ...
        present in the loaded file. This test mainly serves to check that all
        items expected to be in the checkpoint file are indeed there, nothing
        is being corrupted, and any changes to checkpointing do not break any
        existing features.
        """
        # create one delta, just to have a checkpoint file
        p = utilities.yaml_from_dict(
...
class TestSettingConstants:
    ...
    def test_set_constant_g(self, tmp_path: Path) -> None:
        """
        check gravity
        """
    ...
    def test_set_constant_distances(self, tmp_path: Path) -> None:
        """
        check distances
        """
        # create delta from default options
    ...
    def test_set_ivec(self, tmp_path: Path) -> None:
        # create delta from default options
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.ivec[0, 0] == pytest.approx(-np.sqrt(0.5))

    def test_set_jvec(self, tmp_path: Path) -> None:
        # create delta from default options
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.jvec[0, 0] == pytest.approx(-np.sqrt(0.5))

    def test_set_iwalk(self, tmp_path: Path) -> None:
        # create delta from default options
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.iwalk[0, 0] == -1

    def test_set_jwalk(self, tmp_path: Path) -> None:
        # create delta from default options
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.jwalk[0, 0] == -1

    def test_kernel1(self, tmp_path: Path) -> None:
        # create delta from default options
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
        assert delta.kernel1[0, 0] == 1

    def test_kernel2(self, tmp_path: Path) -> None:
        # create delta from default options
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        delta = DeltaModel(input_file=p)
...
class TestSettingParametersFromYAMLFile:
    ...
    def test_hb(self, tmp_path: Path) -> None:
    ...
    def test_lambda(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"sed_lag": 0.8})
        _delta = DeltaModel(input_file=p)
        assert _delta._lambda == 0.8

    def test_alpha(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"alpha": 0.25})
        _delta = DeltaModel(input_file=p)
        assert _delta.alpha == 0.25

    def test_stepmax_default(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"alpha": 0.25})
        _delta = DeltaModel(input_file=p)
        assert _delta.stepmax == 2 * (_delta.L + _delta.W)

    def test_stepmax_integer(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"stepmax": 10})
        _delta = DeltaModel(input_file=p)
        assert _delta.stepmax == 10

    def test_stepmax_float(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"stepmax": 11.0})
        _delta = DeltaModel(input_file=p)
        assert _delta.stepmax == 11

    def test_save_eta_grids(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"save_eta_grids": True})
        _delta = DeltaModel(input_file=p)
    ...
    def test_save_depth_grids(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"save_depth_grids": True})
        _delta = DeltaModel(input_file=p)
        assert _delta._save_any_grids is True
        assert len(_delta._save_fig_list) == 0
    ...
    def test_save_stage_grids(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"save_stage_grids": True})
        _delta = DeltaModel(input_file=p)
        assert _delta._save_any_grids is True
        assert len(_delta._save_fig_list) == 0
    ...
    def test_save_discharge_grids(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"save_discharge_grids": True}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta._save_any_grids is True
        assert len(_delta._save_fig_list) == 0
    ...
    def test_save_velocity_grids(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"save_velocity_grids": True}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta._save_any_grids is True
        assert len(_delta._save_fig_list) == 0
    ...
    def test_save_sedflux_grids(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"save_sedflux_grids": True}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta._save_any_grids is True
        assert len(_delta._save_fig_list) == 0
    ...
    def test_save_sandfrac_grids(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"save_sandfrac_grids": True}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta._save_any_grids is True
        assert len(_delta._save_fig_list) == 0
    ...
    def test_save_discharge_components(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"save_discharge_components": True}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta._save_any_grids is True
        assert len(_delta._save_fig_list) == 0
    ...
    def test_save_velocity_components(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"save_velocity_components": True}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta._save_any_grids is True
        assert len(_delta._save_fig_list) == 0
    ...
    def test_save_eta_figs(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"save_eta_figs": True})
        _delta = DeltaModel(input_file=p)
        assert len(_delta._save_fig_list) > 0
        assert _delta._save_any_grids is False

    def test_save_depth_figs(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"save_depth_figs": True})
        _delta = DeltaModel(input_file=p)
        assert len(_delta._save_fig_list) > 0
        assert _delta._save_any_grids is False

    def test_save_stage_figs(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"save_stage_figs": True})
        _delta = DeltaModel(input_file=p)
        assert len(_delta._save_fig_list) > 0
        assert _delta._save_any_grids is False

    def test_save_discharge_figs(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"save_discharge_figs": True}
        )
        _delta = DeltaModel(input_file=p)
        assert len(_delta._save_fig_list) > 0
        assert _delta._save_any_grids is False

    def test_save_velocity_figs(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"save_velocity_figs": True}
        )
        _delta = DeltaModel(input_file=p)
        assert len(_delta._save_fig_list) > 0
        assert _delta._save_any_grids is False

    def test_save_sedflux_figs(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"save_sedflux_figs": True}
        )
        _delta = DeltaModel(input_file=p)
        assert len(_delta._save_fig_list) > 0
        assert _delta._save_any_grids is False

    def test_save_sandfrac_figs(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"save_sandfrac_figs": True}
        )
        _delta = DeltaModel(input_file=p)
        assert len(_delta._save_fig_list) > 0
        assert _delta._save_any_grids is False

    def test_save_figs_sequential(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"save_figs_sequential": False}
        )
        _delta = DeltaModel(input_file=p)
        assert len(_delta._save_fig_list) == 0
        assert _delta._save_any_grids is False
    ...
    def test_toggle_subsidence(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"toggle_subsidence": True}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta.toggle_subsidence is True

    def test_start_subsidence(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"start_subsidence": 12345}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta.start_subsidence == 12345

    def test_subsidence_rate(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"toggle_subsidence": True, "subsidence_rate": 1e-9}
        )
        _delta = DeltaModel(input_file=p)
    ...
    def test_sand_frac_bc(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"sand_frac_bc": -1})
        _delta = DeltaModel(input_file=p)
        assert _delta.sand_frac_bc == -1


class TestSettingOtherParametersFromYAMLSettings:
    def test_theta_sand(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"coeff_theta_sand": 1.4, "theta_water": 1.2}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta.theta_sand == 1.68

    def test_theta_mud(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"coeff_theta_mud": 0.8, "theta_water": 1.3}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta.theta_mud == 1.04

    def test_U_dep_mud(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"coeff_U_dep_mud": 0.4325, "u0": 2.2}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta.U_dep_mud == 0.9515

    def test_U_ero_sand(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"coeff_U_ero_sand": 1.23, "u0": 2.2}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta.U_ero_sand == 2.706

    def test_U_ero_mud(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"coeff_U_ero_mud": 1.67, "u0": 2.2}
        )
        _delta = DeltaModel(input_file=p)
    ...
    def test_W(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"Width": 1200, "dx": 20})
        _delta = DeltaModel(input_file=p)
        assert _delta.W == 60

    def test_u_max(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"u0": 2.3})
        _delta = DeltaModel(input_file=p)
        assert _delta.u_max == 4.6  # == 2*u0

    def test_C0(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"C0_percent": 10})
        _delta = DeltaModel(input_file=p)
        assert _delta.C0 == 0.1

    def test_dry_depth(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"h0": 0.5})
        _delta = DeltaModel(input_file=p)
        assert _delta.dry_depth == 0.05

    def test_dry_depth_limiter(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"h0": 20})
        _delta = DeltaModel(input_file=p)
        assert _delta.dry_depth == 0.1

    def test_CTR(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"Length": 4000, "Width": 6000, "dx": 10}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta.CTR == 299  # 300th index

    def test_CTR_small(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"Length": 40, "Width": 40, "dx": 10}
        )
        _delta = DeltaModel(input_file=p)
        # small case, instead of 4/2-1=1 it is 4/2=2
        assert _delta.CTR == 2

    def test_gamma(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
    ...
    def test_V0(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"h0": 3, "dx": 15})
        _delta = DeltaModel(input_file=p)
    ...
    def test_Qw0(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path,
            "input.yaml",
            {"u0": 0.8, "h0": 2, "N0_meters": 500, "Width": 6000, "dx": 5},
        )
        _delta = DeltaModel(input_file=p)
        assert _delta.N0 == 100
    ...
    def test_dVs(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path,
            "input.yaml",
            {"u0": 0.8, "h0": 2, "N0_meters": 500, "Width": 6000, "dx": 5},
    ...
    def test_Qs0(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path,
            "input.yaml",
            {
    ...
    def test_Vp_sed(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path,
            "input.yaml",
            {
    ...
    def test_stepmax_and_size_indices(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"Length": 1600, "Width": 1200, "dx": 20}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta.L == 80
        assert _delta.W == 60
    ...
    def test_dt(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path,
            "input.yaml",
            {
    ...
    def test_diffusion_multiplier(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path,
            "input.yaml",
            {
    ...
    def test_active_layer_thickness_float(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"active_layer_thickness": 2.7}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta.active_layer_thickness == 2.7

    def test_active_layer_thickness_int(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"active_layer_thickness": 2}
        )
        _delta = DeltaModel(input_file=p)
        assert _delta.active_layer_thickness == 2

    def test_active_layer_thickness_default(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml")
        _delta = DeltaModel(input_file=p)
...
class TestInputParameterResolution:
    def test_parameter_yaml_and_kwargs(self, tmp_path):
        ...
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"u0": 0.8})
    ...
    def test_parameter_yaml_unused(self, tmp_path):
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"unusable": 0.8})
    ...
    def test_parameter_yaml_forbidden(self, tmp_path):
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"matrix": 0.8})
        with pytest.warns(UserWarning, match=r"A Preprocessor-only .*matrix.*"):
            _ = DeltaModel(input_file=p)
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"ensemble": 5})
        with pytest.warns(UserWarning, match=r"A Preprocessor-only .*ensemble.*"):
            _ = DeltaModel(input_file=p)
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"set": "whatever"})
...
class TestInitMetadataList:
    def test_save_list_exists(self, tmp_path: Path) -> None:
        file_name = "user_parameters.yaml"
        p, f = utilities.create_temporary_file(tmp_path, file_name)
        utilities.write_parameter_to_file(f, "out_dir", tmp_path / "out_dir")
        f.close()
    ...
    def test_default_meta_list(self, tmp_path: Path) -> None:
        file_name = "user_parameters.yaml"
        p, f = utilities.create_temporary_file(tmp_path, file_name)
        utilities.write_parameter_to_file(f, "out_dir", tmp_path / "out_dir")
